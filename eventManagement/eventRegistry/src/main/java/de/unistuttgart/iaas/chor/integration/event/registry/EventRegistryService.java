	...
	public void assignProcessModel(SCEInstanceRef sceInstance, ProcessModel model) {
		...
		if (model != null) {
			...
			if (!sceInstance.getProcessModels().contains(model)) {
				sceInstance.getProcessModels().add(model);
				model.getSceInstanceRefs().add(sceInstance);
			}
		}
	}
...
//			SCEInstanceRef instanceRef = em.find(SCEInstanceRef.class, sceInstanceID);
...
//	
//	public ActivityInstance getOrCreateActivityInstanceByCompositeKey(ProcessInstance processInstance, Long scopeID, String scopeXPath, Long activityID, String activityXPath) {
//		TypedQuery<ActivityInstance> q1 = em.createQuery("SELECT a FROM ActivityInstance a WHERE (a.processInstance = :procInstance) AND (a.scopeID = :scopeID OR a.scopeID IS NULL) AND (a.scopeXPath = :scopeXPath OR a.scopeXPath IS NULL) AND (a.activityID = :activityID OR a.activityID IS NULL) AND (a.activityXPath = :activityXPath OR a.activityXPath IS NULL)", ActivityInstance.class);
...
//	public ActivityInstance getOrCreateActivityInstance(SCEInstanceRef sceInstanceRef, ProcessInstance processInstance, String processModelID, Long scopeID, String scopeXPath, Long activityID, String activityXPath) {
//		
//		ProcessModel processModel = em.find(ProcessModel.class, processModelID);
//
//		ActivityInstancePK activityInstancePK = new ActivityInstancePK(sceInstanceRef, processModel, processInstance, activityID);
//		
...
//	}
	...
	public ActivityInstance getOrCreateActivityInstance(SCEInstanceRef sceInstanceRef, ProcessInstance processInstance, String processModelID, Long scopeID, String scopeXPath, Long activityID, String activityXPath, Long timestamp, String elementModelID) {
...
//		ProcessModel processModel = em.find(ProcessModel.class, processModelID);
	...
	public String getSCEInstanceOfProcessInstance(String instanceId) {
		...
		if (instanceId != null) {
			ProcessInstance processInstance = em.find(ProcessInstance.class, instanceId);
	...
	public Collection<String> getSCEInstanceOfProcessModel(String modelId) {
		...
		if (modelId != null) {
			...
			if (processModel != null) {
				Collection<SCEInstanceRef> instances = processModel.getSceInstanceRefs();

				if (instances != null) {
					for (SCEInstanceRef instance : instances) {
						sceInstanceIds.add(instance.getSceInstanceId());
					}
				}
			}
		}
	...
	@Override
	public MessageLink createMessageLink(String messageLinkID, String messageLinkName, String sendingProcessModelID, String receivingProcessModelID, String sendingActivityModelID, ActivityTypeEnum sendingActivityType, String receivingActivityModelID, ActivityTypeEnum receivingActivityType) {
	...
	public List<MessageLinkInstance> getMessageLinkInstances(ChoreographyInstance choreographyInstance, ActivityInstance activityInstance) {
		...
		TypedQuery<MessageLinkInstance> q1 = em.createQuery("SELECT mi FROM MessageLinkInstance mi WHERE mi.choreographyInstance.choreographyInstanceID = :choreographyInstanceID AND " +
				"(mi.sendingActivityInstance.compositePrimaryKey.activityInstanceID = :activityInstanceID OR mi.receivingActivityInstance.compositePrimaryKey.activityInstanceID = :activityInstanceID)", MessageLinkInstance.class);
		q1.setParameter("choreographyInstanceID", choreographyInstance.getChoreographyInstanceID());
	...
	public List<MessageLinkInstance> getMessageLinkInstancesOfChoreographyModel(ChoreographyInstance choreographyInstance) {
		...
		TypedQuery<MessageLinkInstance> q1 = em.createQuery("SELECT m FROM MessageLinkInstance m WHERE m.choreographyInstance.choreographyInstanceID = :choreographyInstanceID" , MessageLinkInstance.class);
		q1.setParameter("choreographyInstanceID", choreographyInstance.getChoreographyInstanceID());
	...
	public Iteration createIteration(String iterationID, RepetitionMethodEnum repetitionMethod, ActivityInstance initialRewindingPoint, ProcessInstance initialStartParticipant,
			Map<ProcessInstance, Tuple> rewindingPointMap, List<MessageLinkInstance> messageLinkInstances, ChoreographyInstance choreographyInstance) {
	...
	public ProcessInstance getOldestUnRepeatedProcessInstance(ProcessModel processModel, RewindingPoints rewindingPoints) {
		...
		TypedQuery<ProcessInstance> q1 = em.createQuery("SELECT p FROM ProcessInstance p WHERE p.creationTimestamp = (SELECT MAX(p.creationTimestamp) FROM ProcessInstance p WHERE p.compositePrimaryKey.processInstanceID IN (SELECT e.processInstance.compositePrimaryKey.processInstanceID " +
				"FROM RewindingPointsEntry e WHERE e.processInstance.processModel.modelId = :modelID AND e.repeated = 'false' AND e.rewindingPoints.rewindingPointsID = :rewindingPointsID))" , ProcessInstance.class);
...
//		
//		
//		TypedQuery<CorrelationInstance> query = em.createQuery("SELECT ci FROM CorrelationInstance ci WHERE ci.correlation.correlationID = :correlationID " +
...
//	}
	...
	public CorrelationSetInstance getOrCreateCorrelationSetInstance(CorrelationSet correlationSet, String propertyValue, ProcessInstance processInstance) {
		...
		TypedQuery<CorrelationSetInstance> query = em.createQuery("SELECT csi FROM CorrelationSetInstance csi WHERE csi.correlationSet.correlationSetID = :correlationSetID " +
				"AND csi.processInstance.compositePrimaryKey.processInstanceID = :processInstanceID " +
	...
	public void assignPackageNameToProcessModel(QName modelQName, String packageName) {
		...
		TypedQuery<ProcessModel> query = em.createQuery("SELECT m FROM ProcessModel e WHERE m.modelQName = :modelQName" , ProcessModel.class);
		query.setParameter("modelQName", modelQName);
		
		ProcessModel processModel =null;
		
		if(!query.getResultList().isEmpty()){
			 processModel = query.getResultList().get(0);
		}
	...
	public Collection<ProcessInstanceEvent> getProcessInstanceEvents(ProcessInstance processInstance) {
		...
		processInstance = em.find(ProcessInstance.class, processInstance.getCompositePrimaryKey().getProcessInstanceID());
		
		return processInstance.getProcessInstanceEvents();
